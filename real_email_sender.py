...
# -*- coding: utf-8 -*-
"""
Email Sender for Real Financial News
Reads the generated real news file and sends it via email
"""
...
def find_latest_real_news_file():
    """Find the latest real financial news file"""
    # 优先查找改进版文件
    list_of_files = glob.glob('improved_financial_news_*.txt')
    if not list_of_files:
        # 如果没有改进版，查找原始版本
        list_of_files = glob.glob('real_financial_news_*.txt')
    if not list_of_files:
        return None
...
def main():
    ...
    news_file = find_latest_real_news_file()
    if not news_file:
        print("Error: No real financial news file found")
        return

    print(f"找到真实新闻文件: {news_file}")
